...
const CalendarScreen = () => {
  ...
  const DayComponent = React.memo(({ date, state }) => {
    const [dayProgress, setDayProgress] = useState(null);
    const isFuture = !isTodayOrPast(date.dateString);
    ...
    useEffect(() => {
      ...
      const loadProgress = async () => {
        // Only load progress for past/today dates
        if (!isFuture) {
          const progress = await getDayProgress(date.dateString);
          if (isMounted) {
            setDayProgress(progress);
          }
        } else {
          if (isMounted) setDayProgress(0);
        }
      };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Calendar
          ...
          onDayPress={(day) => {
            if (isTodayOrPast(day.dateString)) {
              navigation.navigate('JobDetails', { selectedDate: day.dateString });
            }
          }}
          // Remove maxDate to allow viewing future months
